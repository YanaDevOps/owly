...
func cspHeader(w http.ResponseWriter, connect string) {
	...
	w.Header().Add("Content-Security-Policy",
		c+"img-src blob: data: 'self'; media-src blob: 'self'; script-src 'unsafe-eval' 'self'; default-src 'self'")
...
// parseGroupNameWithPassword extracts the group name from a path that may contain
// an obfuscated password, e.g. "/group/owl/FBgODAI." returns "owl"
func parseGroupNameWithPassword(prefix string, p string) string {
	...
	// Split by '/' and get the first segment (group name)
	parts := strings.Split(rest, "/")
	if len(parts) == 0 {
		return ""
	}

	// First segment is the group name
	name := parts[0]
	if name == "" {
		return ""
	}
...
func groupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if kind == ".status" && rest == "" {
		...
		// kind starts with '.' but is not a known special value
		notFound(w)
		return
	}
	...
	if err != nil {
		// Fallback for password-in-URL invites: /group/<name>/<password>.
		// We try this only when the full parsed group path doesn't resolve,
		// so subgroup URLs keep working as before.
		if errors.Is(err, os.ErrNotExist) {
			name2 := parseGroupNameWithPassword("/group/", r.URL.Path)
			if name2 != "" && name2 != name {
				g2, err2 := group.Add(name2, nil)
				if err2 == nil {
...
func wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("[ws] Upgrade failed remote=%s host=%s xff=%q ua=%q err=%v",
			r.RemoteAddr, r.Host, r.Header.Get("X-Forwarded-For"), r.UserAgent(), err)
		return
	}
	log.Printf("[ws] Upgrade accepted remote=%s host=%s xff=%q ua=%q",
		r.RemoteAddr, r.Host, r.Header.Get("X-Forwarded-For"), r.UserAgent())
