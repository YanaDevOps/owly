...
func wsCloseSuffix(err error) string {
	if err == nil {
		return ""
	}
	var closeErr *websocket.CloseError
	if errors.As(err, &closeErr) {
		return fmt.Sprintf(" (ws code=%d text=%q)", closeErr.Code, closeErr.Text)
	}
	return ""
}
...
type clientMessage struct {
	...
	Reactions        *group.ChatReactions     `json:"reactions,omitempty"`
}

type chatReactionValue struct {
	Id       string         `json:"id"`
	Counts   map[string]int `json:"counts,omitempty"`
	Selected *string        `json:"selected,omitempty"`
}

var allowedChatReactions = map[string]struct{}{
	"👍":  {},
	"❤️": {},
	"😂":  {},
	"😮":  {},
...
func reactionPayload(g *group.Group, messageId, userId string) *group.ChatReactions {
	return g.GetChatReactions(messageId, userId)
}
...
func StartClient(conn *websocket.Conn, addr net.Addr) (err error) {
	...
	if err != nil {
		log.Printf("[ws] Handshake read failed remote=%s err=%v%s",
			clientAddrString(addr), err, wsCloseSuffix(err))
		conn.Close()
		return
	}

	if m.Type != "handshake" {
		log.Printf("[ws] Protocol error before handshake remote=%s type=%q",
			clientAddrString(addr), m.Type)
	...
	log.Printf("[ws] Handshake accepted client=%s remote=%s versions=%v",
		c.id, clientAddrString(c.addr), m.Version)
	go clientWriter(conn, c.id, c.addr, c.writeCh, c.writerDone)
	defer func() {
		sessionAge := time.Since(started).Round(time.Millisecond)
		m, e := errorToWSCloseMessage(c.id, err)
		if isWSNormalError(err) {
			if c.group != nil {
				log.Printf("Client %s (%s@%s) disconnected normally after %v%v",
					c.id, c.username, c.group.Name(), sessionAge, wsCloseSuffix(err))
			} else {
				log.Printf("Client %s disconnected normally after %v%v",
					c.id, sessionAge, wsCloseSuffix(err))
			}
			err = nil
		} else if _, ok := err.(group.KickError); ok {
			if c.group != nil {
				log.Printf("Client %s (%s@%s) kicked after %v: %v%v",
					c.id, c.username, c.group.Name(), sessionAge, err, wsCloseSuffix(err))
			} else {
				log.Printf("Client %s kicked after %v: %v%v",
					c.id, sessionAge, err, wsCloseSuffix(err))
			}
			err = nil
		} else if err != nil {
			if c.group != nil {
				log.Printf("Client %s (%s@%s) disconnected with error after %v: %v%v",
					c.id, c.username, c.group.Name(), sessionAge, err, wsCloseSuffix(err))
			} else {
				log.Printf("Client %s disconnected with error after %v: %v%v",
					c.id, sessionAge, err, wsCloseSuffix(err))
			}
		}
		if m != nil {
...
func clientLoop(c *webClient, ws *websocket.Conn, versionError bool) error {
	...
	go clientReader(ws, c.id, c.addr, read, c.done)
	...
	for {
		...
		case m, ok := <-read:
			if !ok {
				log.Printf("[ws-loop] Reader channel closed client=%s remote=%s",
					c.id, clientAddrString(c.addr))
			...
			case error:
				log.Printf("[ws-loop] Reader error client=%s remote=%s err=%v%s",
					c.id, clientAddrString(c.addr), m, wsCloseSuffix(m))
				return m
			}
		case <-c.actions.Ch:
		...
		case <-ticker.C:
			...
			if idle > deadClientTimeout {
				log.Printf("[ws-loop] Dead client timeout client=%s remote=%s idle=%v timeout=%v",
					c.id, clientAddrString(c.addr), idle.Round(time.Second), deadClientTimeout)
				return errors.New("client is dead")
			}
			// Some reverse proxies timeout connexions at 60
			// seconds, make sure we generate some activity
			if idle > pingAfterIdle {
				err := c.write(clientMessage{
					Type: "ping",
				})
				if err != nil {
					log.Printf("[ws-loop] Ping write failed client=%s remote=%s idle=%v err=%v",
						c.id, clientAddrString(c.addr), idle.Round(time.Second), err)
					return err
				}
			}
		}
		...
		if a.kind == "join" {
			...
			for _, m := range h {
				err := c.write(clientMessage{
					...
					Value:     m.Value,
					Kind:      m.Kind,
					Reactions: reactionPayload(g, m.Id, c.id),
				})
				if err != nil {
					return err
				}
...
func handleClientMessage(c *webClient, m clientMessage) error {
	...
	case "join":
		...
		if err != nil {
			...
			if errors.Is(err, token.ErrUsernameRequired) {
				...
				log.Printf("Join group: not authorised: %v (client=%s, username=%s, group=%s)",
					err, c.id, username, m.Group)
	...
	case "chat", "usermessage":
		...
		if m.Type == "chat" && m.Dest == "" {
			mm.Reactions = reactionPayload(g, id, "")
		}
		if m.Dest == "" {
	...
	case "groupaction":
		...
		case "reactchat":
			if !member("message", c.permissions) && !member("present", c.permissions) {
				return c.error(group.UserError("not authorised"))
			}

			value, ok := m.Value.(map[string]interface{})
			if !ok {
				return c.error(group.UserError("bad value in reactchat"))
			}
			messageId, _ := value["id"].(string)
			if messageId == "" {
				return c.error(group.UserError("bad value in reactchat"))
			}

			var emoji *string
			if raw, found := value["emoji"]; found && raw != nil {
				...
				if choice != "" {
					if _, ok := allowedChatReactions[choice]; !ok {
						return c.error(group.UserError("unsupported reaction"))
					}
					emoji = &choice
				}
			}

			err := g.SetChatReaction(messageId, c.id, emoji)
			if errors.Is(err, os.ErrNotExist) {
				return c.error(group.UserError("message unknown"))
			}
			if err != nil {
				return c.error(err)
			}
			...
			for _, client := range g.GetClients(nil) {
				cc, ok := client.(*webClient)
				if !ok {
					continue
				}
				reactions := reactionPayload(g, messageId, cc.id)
				msg := chatReactionValue{Id: messageId}
				if reactions != nil {
					msg.Counts = reactions.Counts
					msg.Selected = reactions.Selected
				}
				err := cc.write(clientMessage{
					Type:  "usermessage",
					Kind:  "chatreaction",
					Value: msg,
				})
				if err != nil {
					log.Printf("chatreaction: %v", err)
				}
			}
		case "lock", "unlock":
			if !member("op", c.permissions) {
...
func clientReader(conn *websocket.Conn, clientID string, addr net.Addr, read chan<- interface{}, done <-chan struct{}) {
	defer close(read)
	for {
		var m clientMessage
		err := conn.ReadJSON(&m)
		if err != nil {
			log.Printf("[ws-reader] ReadJSON failed client=%s remote=%s err=%v%s",
				clientID, clientAddrString(addr), err, wsCloseSuffix(err))
			select {
			case read <- err:
				return
			case <-done:
...
func clientWriter(conn *websocket.Conn, clientID string, addr net.Addr, ch <-chan interface{}, done chan<- struct{}) {
	defer func() {
		close(done)
		conn.Close()
	}()
	...
	for {
		...
		if err != nil {
			log.Printf("[ws-writer] SetWriteDeadline failed client=%s remote=%s err=%v",
				clientID, clientAddrString(addr), err)
			return
		}
		switch m := m.(type) {
		case clientMessage:
			...
			if err != nil {
				log.Printf("[ws-writer] WriteJSON failed client=%s remote=%s type=%s err=%v%s",
					clientID, clientAddrString(addr), m.Type, err, wsCloseSuffix(err))
				return
			}
		case []byte:
			err := conn.WriteMessage(websocket.TextMessage, m)
			if err != nil {
				log.Printf("[ws-writer] WriteMessage failed client=%s remote=%s len=%d err=%v%s",
					clientID, clientAddrString(addr), len(m), err, wsCloseSuffix(err))
				return
			}
		case closeMessage:
			if m.data != nil {
				...
				if err != nil {
					log.Printf("[ws-writer] Write close frame failed client=%s remote=%s err=%v%s",
						clientID, clientAddrString(addr), err, wsCloseSuffix(err))
				}
			}
			return
...
func (c *webClient) Close() error {
	return c.close(websocket.FormatCloseMessage(
		websocket.CloseNormalClosure,
		"session replaced",
	))
}
